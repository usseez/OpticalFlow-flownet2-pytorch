...
def estimate_F_from_flow(flow_torch, n_points=2000, ransac_thresh=1.0):
        flow = flow_torch.detach().cpu().numpy()  # (2,H,W)
        u, v = flow[0], flow[1]
        H, W = u.shape

        yy, xx = np.meshgrid(np.arange(H), np.arange(W), indexing='ij')
        x1 = np.stack([xx, yy], axis=-1).reshape(-1, 2).astype(np.float32)
        x2 = np.stack([xx + u, yy + v], axis=-1).reshape(-1, 2).astype(np.float32)
        ...
        if len(x1) > n_points:
            idx = np.random.choice(len(x1), n_points, replace=False)
            x1, x2 = x1[idx], x2[idx]

        Fmat, _ = cv2.findFundamentalMat(x1, x2, cv2.FM_RANSAC, ransac_thresh, 0.99, 2000)
        if Fmat is None or Fmat.shape != (3,3):
            return torch.zeros(3,3, dtype=torch.float32)
        return torch.from_numpy(Fmat.astype(np.float32))
...
class MpiSintel(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        F = estimate_F_from_flow(flow)

        return [images], [flow, F]
...
class MyUnlabeledFlowDataset(data.Dataset):
    """
    Example: each sample returns:
      data  = [stacked_images]           # (6, H, W)
      target = [proxy_flow, F]          # proxy_flow: (2, H, W), F: (3, 3) or (9,)
    """
    def __init__(self, args, is_cropped, root, list_file, **kwargs):
        self.root = root
        self.is_cropped = is_cropped
        self.rgb_max = args.rgb_max

        # list_file contains relative paths for pairs, proxy flows & F, etc.
        # You can adapt this to however you store your data.
        with open(list_file, 'r') as f:
            self.samples = [line.strip().split() for line in f]
        # each line example:
        #    img1_path img2_path proxy_flow_path F_path
    ...
    def _load_flow(self, path):
        # adapt this to however you store pseudo flow (.flo, .npy, etc.)
        full_path = os.path.join(self.root, path)
        arr = np.load(full_path)   # (H, W, 2) for example
        flow = torch.from_numpy(arr.transpose(2, 0, 1))  # (2, H, W)
        return flow

    def _load_F(self, path):
        full_path = os.path.join(self.root, path)
        F = np.load(full_path)     # (3, 3) or (9,)
        F = torch.from_numpy(F.astype(np.float32))
        return F

    def __getitem__(self, idx):
        img1_path, img2_path, proxy_flow_path, F_path = self.samples[idx]

        img1 = self._load_image(img1_path)
        img2 = self._load_image(img2_path)
        proxy_flow = self._load_flow(proxy_flow_path)
        F = self._load_F(F_path)

        # # normalize images like other FlowNet2 datasets
        # img1 = img1 / self.rgb_max
        # img2 = img2 / self.rgb_max

        # # stack along channel dimension: (6, H, W)
        # stacked = torch.cat([img1, img2], dim=0)

        # # pack as lists to match your main script
        # data = [stacked]


        images = torch.stack([img1, img2], dim=1)  # (3,2,H,W)
        data = [images]
