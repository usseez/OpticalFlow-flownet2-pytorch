...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', '-s', default='./weights/251215', type=str, help='directory for saving')
    ...
    tools.add_arguments_for_module(parser, losses, argument_for_class='loss', default='L1Loss')   #--loss ProxyLabelLoss --loss_epipolar_thresh 1.0 --loss_proxy_weight 1.0 --loss_smooth_weight 0.1 when execute
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            losses = model(data[0], target)
            losses = [torch.mean(loss_value) for loss_value in losses] 
            loss_val = losses[0] # Collect first loss for weight update
            total_loss += loss_val.item()
            loss_values = [v.item() for v in losses]
            logger.info(f"Epoch:{epoch}, Train Losses:{loss_values}")
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            with torch.no_grad():
                losses, output = model(data[0], target, inference=True)

            losses = [torch.mean(loss_value) for loss_value in losses] 
            loss_val = losses[0] # Collect first loss for weight update
            total_loss += loss_val.item()
            loss_values = [v.item() for v in losses]
            print(f"Epoch:{epoch}, Train Losses:{loss_values}")
            # gather loss_labels, direct return leads to recursion limit error as it looks for variables to gather'
            loss_labels = list(model.module.loss.loss_labels)
    ...
    for epoch in progress:
        ...
        if not args.skip_validation and ((epoch - 1) % args.validation_frequency) == 0:
            ...
            tools.save_checkpoint({   'arch' : args.model,
                                      ...
                                      'best_EPE': best_err}, 
                                      is_best, args.save, f'{epoch}_{args.model}')
        ...
        if not args.skip_training:
            ...
            if ((epoch - 1) % args.validation_frequency) == 0:
                ...
                tools.save_checkpoint({   'arch' : args.model,
                                          ...
                                          'state_dict': model_and_loss.module.model.state_dict(),
                                          'best_EPE': train_loss}, 
                                          False, args.save, args.model, filename = f'train-checkpoint{epoch}.pth.tar')
                checkpoint_progress.update(1)
                checkpoint_progress.close()
